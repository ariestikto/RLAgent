/**
 * 
 */
package marketFramework;

/**
 * @author pa1g15
 *
 */
public class Market {
	public static final double START_PRICE = 5; // p/kWh
	public static final double ELECTRICITY_SUPPLY = 65; // kWh
//	public static final double ELECTRICITY_SUPPLY = 18; // kWh
	public static final int AUCTION_PARTICIPANT = 11;
	public static final int TOTAL_CAR_TYPE = 7;
	public static final double AGENT_LEARNING_RATE = 0.2; // 0.2
	public static final double AGENT_DISCOUNT_FACTOR = 0.95; // 0.8
	public static final double EPSILON_PARAMETER = 0.1; // 0.1
}
